	...
	readSG := func(gd *grid.Definition, sgfp string) {
		...
		loadIndx := func(fp string) ([]int, []int) {
			if _, ok := fileExists(fp); !ok {
				panic(fmt.Sprintf("buildMapper.readSG.loadIndx file not found: %s", fp))
			}
			...
			for i, c := range s.Cids {
				if v, ok := g.A[c]; ok {
					...
					panic("buildMapper.readSG.loadIndx error: " + fp)
				}
			}
	...
	readGW := func(gd *grid.Definition, gwfp string) {
		...
		if gwfp == "" { // all 1 GW zone
			...
			loadIndx := func(fp string) (map[int]int, []int) {
				if _, ok := fileExists(fp); !ok {
					panic(fmt.Sprintf("buildMapper.readGW.loadIndx file not found: %s", fp))
				}
				fmt.Printf("   loading: %s\n", fp)
				var g grid.Indx
				g.GD = gd
			...
			for i, c := range s.Cids {
				if gid, ok := mgw[c]; ok {
					...
					panic("buildMapper.readGW groundwater cellID error")
				}
			}

			if len(nulls) > 0 {
				newGD := s.GD.Copy() // for concurrency issues
				newGD.RemoveActives(nulls)
				newGD.SaveAs(gwfp + "-new.gdef")
				panic(fmt.Sprintf("buildMapper.readGW groundwater ID null value found at cells: %d\n   updated gdef saved to %s", nulls, gwfp+"-new.gdef"))
			}
